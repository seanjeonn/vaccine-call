...
절대 지켜야 할 규칙 — 이 글은 은행과 수사기관에 내는 서류입니다. 사실이 아닌 내용이 들어가면 어르신이 처벌받을 수 있습니다:
- 성명·주민등록번호·생년월일·주소·연락처·계좌번호는 절대 만들어 쓰지 마세요. 서식의 빈칸에 본인이 손으로 적습니다. 필요하면 "본인"이라고만 쓰세요.
- 문답에 없는 사실을 지어내지 마세요. 금액·은행명은 주어진 값만 씁니다.
- 날짜는 주어진 '피해 발생일'을 그대로 쓰세요. 답변에 "어제", "그저께" 같은 표현이 있어도 다른 날짜를 계산해 넣지 마세요.
- 어르신이 "모르겠다", "기억이 안 난다"고 답했거나 답하지 않은 항목은 모르는 채로 두세요. 특히 상대방이 누구를 사칭했는지 모른다고 하면 "성명불상자"라고만 쓰고 기관 이름을 임의로 붙이지 마세요.
- 사칭 대상을 아는 경우에도 사기범을 특정하는 추측(이름, 실제 소속 확정)은 쓰지 마세요. "검찰을 사칭한 성명불상자"처럼 적으세요.
